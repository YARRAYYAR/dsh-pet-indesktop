...
本地构建包：`dist/DSH-Pet Smooth-macOS-arm64-*.zip`（Apple Silicon）。五页控制面板、双栏动作库、搜索时排序、即时预览与取消恢复。保留原版拖拽回弹、快速拖动加速、真实碰撞回弹音、对话框开关和 JSON 导入导出；拖动跟随阶段静音，并支持 3 种回弹音效。使用说明见 [ACTION_LIBRARY.md](ACTION_LIBRARY.md)。源码和完整的 91 段原视频保留在本项目。

左右边缘探头默认开启，可在「互动 → 拖拽手感」或托盘菜单关闭：拖到屏幕边缘松手后，角色会以约 45° 探出；点击会拉直并露出更多，5 秒后回到探头姿态。探头过渡期间才使用短时动画计时器，不增加常驻解码或后台线程。
...
- **动态气泡**：点击回应或主动问候时显示无文字气泡；小尾泡先出现，主体随后平滑展开，数秒后自动收起
- **v0.2.9 气泡表情图**：可选随机显示包内 27 张中文 PNG；仅在气泡出现时按需读取，收起后释放
...
- **边缘探头**：开启后拖到左右屏幕边缘会探出部分角色；点击拉直，超时自动收回，关闭功能会恢复进入前的位置
- **多屏工作区**：使用所有显示器可用工作区的联合区域，支持不同分辨率、缩放比例和跨屏拖动
- **透明穿透**：窗口逐帧按人物 alpha 生成 mask，透明区域鼠标直接穿透到下层窗口
- **右键菜单**：手动播放待机/转向/移动/点击回应/随机动作、切换角色、回到右下角、窗口置顶、不移动、开机自启、预设与自定义大小、退出
...
- **动作候选防重复**：动作池保留多个候选，优先避开最近播放的动作，候选过少时自动回退
...
- **动态气泡**：点击宠物后显示无文字气泡，尾泡与主体按顺序渐进出现，默认约 4.8 秒后自动隐藏。可在右键菜单「互动反馈 → 动态气泡」或设置面板中关闭。
...
- **性格模式**：右键桌宠或状态栏图标 →「性格模式」，可选「高冷 / 安静 / 活泼 / 调皮 / 温柔」。
- **气泡位置**：打开「设置与动作预览」，可以单独控制气泡出现、消失，并输入 X/Y 偏移；调整时即时预览，保存后保留。
...
| 大小            | 画布宽度 160～1280px，数字输入与滑杆联动；保留 320 / 462 / 544 / 640px 预设（默认约 462px） |
| 播放速率        | 调节动画播放速度：1.0x / 1.1x / ... / 2.0x                                               |
| 拖动物理        | 开启后拖动桌宠有惯性/离心感，松手会抛出并带重力反弹衰减                                   |
| 动态气泡        | 点击回应或主动问候时显示无文字气泡；尾泡与主体渐进出现，数秒后自动隐藏                   |
| 退出            | 关闭桌宠                                                                                  |
...
- 不同角色可以有**不同的动作集**：程序会递归扫描 `videos/` 下的子目录，
  按目录自动区分“待机 / 转向 / 移动 / 点击回应 / 拖拽 / 事件动作 / 随机动作”。
- 内置角色会随 exe 一起打包。
- 同时支持用户本地外部扩展：
...
- 托盘「复制体」或桌宠右键菜单可连续增加桌宠复制体；不设应用层数量上限，
  每个复制体独立维护位置、动画和物理状态，媒体缓存限制为当前动作，关闭后立即释放。
- 复制体同样保留「边缘探头（左右贴边）」功能；主桌宠与复制体可分别拖到左右边缘。
...
1. **优先按 `videos/` 下的子目录分类**：
   ...
   - `click/` → 点击回应
   - `drag/` → 拖拽（可选）
   - `events/` → 事件动作（手动选择、收藏或加入播放列表，不进入随机动作池）
   - `random/` → 随机动作
...
无需转码即可直接运行。事件动画的原始下载文件可放在同级 `events/` 目录，
超分母版放在被忽略的 `backups/` 目录；运行时只扫描 `videos/`，因此不会把
原始文件和 1440p 母版读入内存。
...
后续更新：新增「原版随机」，随机动作等概率且允许重复。桌宠大小使用可直接输入的像素数字与蓝色圆角滑杆双向同步，保存后生效。「动作出现间隔」设为 0 时跟随模式；1–3600 秒为两次自动动作开始的最小间隔，当前动画播放完才切换，等待时播放待机。播放列表、手动互动和系统繁忙降频仍优先；无待机素材的外部角色沿用模式选择。

托盘或桌宠右键 →「设置与动作预览」：调整音量（macOS）、动态气泡、性格、画布宽度、动作结束后的等待时间。保存才应用设置；取消不保存。恢复默认只重置本页字段，点击保存生效，保留角色、收藏和位置。动作预览会立即播放，并恢复隐藏或暂停的桌宠。
