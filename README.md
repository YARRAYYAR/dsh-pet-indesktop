...
本地构建包：`dist/DSH-Pet-Control-Panel-macOS-arm64.zip`（Apple Silicon）。六页控制面板、双栏动作库、搜索时排序、即时预览与取消恢复。保留原版拖拽回弹、快速拖动加速、真实碰撞回弹音、对话框开关和 JSON 导入导出；拖动跟随阶段静音，并支持 3 种回弹音效。使用说明见 [ACTION_LIBRARY.md](ACTION_LIBRARY.md)。源码和完整的 91 段原视频保留在本项目。

当前桌面版还提供轻量灵动岛胶囊：显示角色、时间和可见状态；支持单击显示/隐藏桌宠、拖动调整位置、顶部吸附，以及深色/浅色/玻璃三种风格。胶囊独立于 WebM 播放链，只使用一个低频刷新计时器。

左右边缘探头默认关闭，可在「互动 → 拖拽手感」或托盘菜单开启：拖到屏幕边缘松手后，角色会以约 45° 探出；点击会拉直并露出更多，5 秒后回到探头姿态。探头过渡期间才使用短时动画计时器，不增加常驻解码或后台线程。
